...
class VGG19:
    def __init__(self, h, w, trainable):
        ...
        self.x = tf.placeholder(tf.float32, [None, self.img_dimh, self.img_dimw, 3])
...
#        self.loss = self.inference_loss(self.out, self.t)
    ...
    def inference_loss(self, out, t):
        loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(out, tf.one_hot(t, self.n_class)))
        return loss
