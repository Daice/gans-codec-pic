...
def train():
    ...
    # Loading the data
    x_train = load('data/train_feature')
    x_test = load('data/train_label')
...
def validate(x_test, epoch, model, sess):
    for i in range(10):
        label = x_test[i * batch_size:(i + 1) * batch_size]
        mos, fake = sess.run([model.downscaled, model.imitation], feed_dict={model.x: label, model.is_training: False})
        saveimg([mos, fake, label], i, ['Input', 'Output', 'Ground Truth'], epoch)


def saveimg(imgs, mark, label, epoch):
    for i in range(batch_size):
        ...
        for j, img in enumerate(imgs):
            ...
            plt.xlabel(label[j])
        seq_ = "{0:04d}".format(i + 1+batch_size*mark)
        epoch_ = "{0:03d}".format(epoch + 1)
        path = os.path.join('result', epoch_, '{}.jpg'.format(seq_))
        if os.path.exists(os.path.join('result', epoch_)) == False:
            os.mkdir(os.path.join('result', epoch_))
        plt.savefig(path)
        plt.close()
