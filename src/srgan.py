...
class SRGAN:
    ...
    def generator(self, x, is_training, reuse):
        with tf.variable_scope('generator', reuse=reuse):
            with tf.variable_scope('deconv1'):
                x = deconv_layer(x, [3, 3, 64, 3], [self.batch_size, self.img_dim, self.img_dim, 64], 1, 'deconv1')
            x = tf.nn.relu(x)
            shortcut = x
            for i in range(16):
                mid = x
                with tf.variable_scope('block{}a'.format(i+1)):
                    x = deconv_layer(x, [3, 3, 64, 64], [self.batch_size, self.img_dim, self.img_dim, 64], 1)
            ...
            with tf.variable_scope('deconv3'):
                x = deconv_layer(x, [3, 3, 256, 64], [self.batch_size, self.img_dim, self.img_dim, 256], 1)
                #x = pixel_shuffle_layer(x, 2, 64)
                x = tf.nn.relu(x)
            with tf.variable_scope('deconv4'):
                x = deconv_layer(x, [3, 3, 512, 256], [self.batch_size, self.img_dim, self.img_dim, 512], 1)
                #x = pixel_shuffle_layer(x, 2, 16)
                x = tf.nn.relu(x)            
            with tf.variable_scope('deconv5'):
                x = deconv_layer(x, [3, 3, 3, 512], [self.batch_size, self.img_dim, self.img_dim, 3], 1)
    ...
    def inference_lossesp(self, x, imitation, true_output, fake_output):
        def inference_content_loss(x, imitation):
            content_loss  = tf.reduce_mean(tf.abs(x - imitation), name='gene_l1_loss')
            '''
            content_loss = None
            for i in range(len(x_phi)):
                if content_loss is None:
                    content_loss = tf.nn.l2_loss(x[i] - imitation[i])
                else:
                    content_loss = content_loss + tf.nn.l2_loss(x[i] - imitation[i])
            return tf.reduce_mean(content_loss)
            '''
            return content_loss
        ...
        def inference_adversarial_loss(true_output, fake_output):
            ...
            cross_entropy = tf.nn.sigmoid_cross_entropy_with_logits(fake_output, tf.ones_like(fake_output))
            g_loss     = tf.reduce_mean(cross_entropy, name='gene_ce_loss')

            cross_entropy_real = tf.nn.sigmoid_cross_entropy_with_logits(true_output, tf.ones_like(true_output))
            d_real_loss     = tf.reduce_mean(cross_entropy_real, name='disc_real_loss')

            cross_entropy_fake = tf.nn.sigmoid_cross_entropy_with_logits(fake_output, tf.zeros_like(fake_output))
            d_fake_loss     = tf.reduce_mean(cross_entropy_fake, name='disc_fake_loss')
